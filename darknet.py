...
def draw_boxes(detections, image, colors, clsName):
    
    import cv2
    for label, confidence, bbox in detections:
        left, top, right, bottom = bbox2points(bbox)
        cv2.rectangle(image, (left, top), (right, bottom), colors[clsName[int(label)]], 2)
        cv2.putText(image, "{} [{:.2f}]".format(clsName[int(label)], float(confidence)),
                    (left, top - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                    colors[clsName[int(label)]], 2)
    return image
    ...
    for j in range(num):
        for idx, name in enumerate(class_names):
            if detections[j].prob[idx] > 0:
                ...
                predictions.append((idx, detections[j].prob[idx], (bbox)))
    return predictions
